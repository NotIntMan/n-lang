/*!
    ...
    Правила `attributes`, `compound_type, и `struct_body` определены в модуле `syntax_parser::compound_types`.

    Правило `module_path` определено в модуле `syntax_parser::others`.

    Правило `identifier` определено в модуле `parser_basics`.

    Правило `function_definition` определено в модуле `syntax_parser::functions`.
*/
